...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--branch", default="main", help="Branch to clone and audit (default: main)"
    )
    parser.add_argument(
        "--output",
        default="both",
...
def _render_markdown(report: AuditReport) -> str:
    ...
    lines = [
        ...
        (
            ...
            f"**{summary.critical}/{summary.high}/{summary.medium}/"
            f"{summary.low}/{summary.info}**"
        ),
...
async def run_audit(repo: str, branch: str, output: str) -> dict[str, Path]:
    ...
    logger = setup_logging(
        config.log_level, config.reports_dir / "security-auditor.log"
    )
    ...
    if output in {"markdown", "both"}:
        md_path = config.reports_dir / f"{report.audit_id}.md"
        md_path.write_text(_render_markdown(report), encoding="utf-8")
        output_files["markdown"] = md_path

    logger.info(
        "Audit completed; generated %s",
        ", ".join(str(p) for p in output_files.values()),
    )
    return output_files
